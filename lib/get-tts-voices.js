...
const getGandrVoices = async(_client, _logger, _credentials) => {
  /* gandr has a fixed roster of six voices and no listing endpoint,
     so the roster is returned as a static list */
  return {
    voices: [
      {value: 'gandr-mia', name: 'Mia'},
      {value: 'gandr-ava', name: 'Ava'},
      {value: 'gandr-jenny', name: 'Jenny'},
      {value: 'gandr-dane', name: 'Dane'},
      {value: 'gandr-leo', name: 'Leo'},
      {value: 'gandr-lewis', name: 'Lewis'}
    ]
  };
};
...
async function getTtsVoices(client, createHash, retrieveHash, logger, {vendor, credentials}) {
  ...
  assert.ok(['google', 'aws', 'polly', 'gandr'].includes(vendor),
  ...
  switch (vendor) {
    ...
    case 'gandr':
      return getGandrVoices(client, logger, credentials);
    default:
      break;
  }
}
