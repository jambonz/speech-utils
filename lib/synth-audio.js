 ...
 * @param {object} opts.options - extra parameters
...
async function synthAudio(client, logger, stats, { account_sid,
  ...
  assert.ok(['google', 'aws', 'polly', 'microsoft',
    'wellsaid', 'nuance', 'nvidia', 'ibm', 'elevenlabs', 'whisper', 'playht'].includes(vendor) ||
  ...
  if (!cached) {
    ...
    switch (vendor) {
      ...
      case 'whisper':
        ...
        break;
      case 'playht':
        audioBuffer = await synthPlayHT(logger, {credentials, stats, voice, text, options});
        break;
...
const synthPlayHT = async(logger, {credentials, stats, voice, text, options}) => {
  const {api_key, user_id, voice_engine} = credentials;
  try {
    const post = bent('https://play.ht', 'POST', 'buffer', {
