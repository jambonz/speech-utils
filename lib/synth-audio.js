    ...
    switch (vendor) {
      ...
      case vendor.startsWith('custom') ? vendor : 'cant_match_value':
        audioData = await synthCustomVendor(logger,
          {credentials, stats, language, voice, key, text, options, renderForCaching, disableTtsStreaming});
        break;
      default:
...
const synthCustomVendor = async(logger, {
  credentials, options, stats, voice, key, language, text, renderForCaching, disableTtsStreaming
}) => {
  const {vendor, auth_token, custom_tts_url, options: credOpts} = credentials;
  ...
  if (!JAMBONES_DISABLE_TTS_STREAMING && !renderForCaching && !disableTtsStreaming) {
    ...
    params += ',write_cache_file=1';
    // Add custom options to params if they exist
    if (opts && typeof opts === 'object') {
      params += `,options=${JSON.stringify(opts).replace(/,/g, ';')}`;
    }
    params += '}';
  ...
  try {
    ...
    stats.increment('tts.count', ['vendor:custom', 'accepted:no']);
    throw err;
  }
};
