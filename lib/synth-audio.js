...
async function synthAudio(client, createHash, retrieveHash, logger, stats, { account_sid,
  ...
  } else if ('gandr' === vendor) {
    assert.ok(voice, 'synthAudio requires voice when gandr is used');
    assert.ok(credentials.api_key, 'synthAudio requires api_key when gandr is used');
    assert.ok(text.length <= 2000,
      `synthAudio: gandr accepts at most 2000 characters per request, got ${text.length}`);
  }
  ...
  if (cached) {
    ...
    switch (vendor) {
      ...
      case 'resemble':
        ...
        break;
      case 'gandr':
        audioData = await synthGandr(logger, {
...
const synthGandr = async(logger, {credentials, stats, language, voice, text}) => {
  const {api_key} = credentials;

  /* no say: streaming branch yet: that needs a freeswitch module for the vendor,
     so gandr always renders to a file, as wellsaid does */
  try {
    const post = bent('https://tts.gandr.ai', 'POST', 'buffer', {
      'Authorization': `Bearer ${api_key}`,
      'Content-Type': 'application/json'
    });
    /* language is not sent: the service selects it from the input text */
    const audioContent = await post('/v1/audio/speech', {
      model: 'tts-1',
      input: text,
      voice,
      response_format: 'mp3'
    ...
    logger.info({err}, 'synth gandr returned error');
    stats.increment('tts.count', ['vendor:gandr', 'accepted:no']);
    throw err;
  }
};
