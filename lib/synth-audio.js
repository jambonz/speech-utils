...
const synthElevenlabs = async(logger, {credentials, options, stats, language, voice, text}) => {
  ...
  try {
    ...
    /* Properties from 'options' will overwrite the ones from speech credentials */    
    const model_id = opts.model_id || credentials.model_id;
    const voice_id = opts.voice_id || voice;
    const voice_settings = opts.voice_settings || {};
    const query = opts.optimize_streaming_latency ? `?optimize_streaming_latency=${opts.optimize_streaming_latency}` : '';
