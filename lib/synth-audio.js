    ...
    switch (vendor) {
      ...
      case 'microsoft':
        ...
        audioData = await synthMicrosoft(logger, {credentials, stats, language, voice, key, text, deploymentId,
          renderForCaching, disableTtsStreaming, disableTtsCache, options});
        break;
      case 'nuance':
...
const synthMicrosoft = async(logger, {
  ...
  try {
    ...
    const {frame_timeout_interval, rtf_timeout_threshold} = options;
    ...
    if (!JAMBONES_DISABLE_TTS_STREAMING && !JAMBONES_DISABLE_AZURE_TTS_STREAMING &&
      ...
      if (rtf_timeout_threshold) params += `,rtf_timeout_threshold=${rtf_timeout_threshold}`;
      if (frame_timeout_interval) params += `,frame_timeout_interval=${frame_timeout_interval}`;
      params += '}';
      return {
