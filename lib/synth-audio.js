...
async function synthAudio(client, logger, stats, { account_sid,
  vendor, language, voice, gender, text, engine, salt, model, credentials, deploymentId, disableTtsCache, options, elevenlabsOptions
}) {
  let audioBuffer;
  ...
  if (!cached) {
    ...
    switch (vendor) {
      ...
      case 'wellsaid':
        ...
        break;
      case 'elevenlabs':
        audioBuffer = await synthElevenlabs(logger, {credentials, options, stats, language, voice, text, filePath, elevenlabsOptions});
        break;
      case 'whisper':
...
const synthElevenlabs = async(logger, {credentials, options, stats, language, voice, text, elevenlabsOptions}) => {
  const {api_key, options: credOpts} = credentials;
  ...
  try {
    ...
    /* Properties from 'elevenlabsOptions' will overwrite the options from speech credentials */
    const model_id = elevenlabsOptions?.model_id || credentials.model_id;
    const voice_id = elevenlabsOptions?.voice_id || voice;
    const voice_settings = elevenlabsOptions?.voice_settings || opts.voice_settings;
    const optimize_streaming_latency = elevenlabsOptions?.optimize_streaming_latency || opts.optimize_streaming_latency;
    const query = optimize_streaming_latency ? `?optimize_streaming_latency=${optimize_streaming_latency}` : '';
    
    const mp3 = await post(`/v1/text-to-speech/${voice_id}${query}`, {
      text,
      model_id,
      voice_settings: {
        stability: 0.5,
