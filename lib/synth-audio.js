...
async function synthAudio(client, logger, stats, { account_sid,
  ...
  if (['nuance', 'nvidia'].includes(vendor) ||
    ...
    (
      ...
      ['elevenlabs', 'deepgram', 'rimelabs', 'google'].includes(vendor)
    )
  ) {
...
const synthGoogle = async(logger, {credentials, stats, language, voice, gender,
  text, renderForCaching, disableTtsStreaming}) => {
  /* default to using the streaming interface, unless disabled by env var OR we want just a cache file */
  if (!process.env.JAMBONES_DISABLE_TTS_STREAMING && !renderForCaching && !disableTtsStreaming) {
    let params = '';
    params += `{credential=${Buffer.from(JSON.stringify(credentials.credentials)).toString('base64')}`;
    params += ',vendor=google';
    params += `,language=${language}`;
    params += ',write_cache_file=1';
    params += `,gender=${gender || 'SSML_VOICE_GENDER_UNSPECIFIED'}`;
    if (typeof voice === 'object') {
      params += `,model=${voice.model}`;
      params += `,reported_usage=${voice.reportedUsage}`;
    } else {
      params += `,voice=${voice}`;
    }
    params += '}';
